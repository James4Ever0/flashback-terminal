...
func Load(path string) (*Config, *ConfigWithSource, error) {
	...
	cfg.BufferDir = expandPath(cfg.BufferDir)
...
func defaultConfig() (*Config, *ConfigWithSource) {
	...
	src := &ConfigWithSource{
		...
		BufferMode:        Source{Value: cfg.BufferMode, Origin: "default"},
		BufferDir:         Source{Value: cfg.BufferDir, Origin: "default"},
...
func applyYAML(cfg *Config, src *ConfigWithSource, fileCfg *FileConfig) {
	...
	if fileCfg.BufferMode != "" {
		cfg.BufferMode = fileCfg.BufferMode
		src.BufferMode = Source{Value: cfg.BufferMode, Origin: "config"}
	}
	if fileCfg.BufferDir != "" {
		cfg.BufferDir = fileCfg.BufferDir
		src.BufferDir = Source{Value: cfg.BufferDir, Origin: "config"}
	}
	if fileCfg.DeviceID != "" {
...
func applyEnvOverrides(cfg *Config, src *ConfigWithSource) {
	...
	if v := os.Getenv("FLASHBACK_SHELL_PTY_BUFFER_MODE"); v != "" {
		cfg.BufferMode = v
		src.BufferMode = Source{Value: v, Origin: "env"}
	}
	if v := os.Getenv("FLASHBACK_SHELL_PTY_BUFFER_DIR"); v != "" {
		cfg.BufferDir = v
		src.BufferDir = Source{Value: v, Origin: "env"}
	}
