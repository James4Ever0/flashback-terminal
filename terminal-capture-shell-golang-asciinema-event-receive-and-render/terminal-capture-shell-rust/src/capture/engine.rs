...
impl Engine {
    ...
    pub fn process(
        ...
        let (capture_ansi, capture_text) = if self.diff_only {
            let prev_text = self.load_prev(session_id, pane_id).unwrap_or_default();
    ...
    pub fn save_hashes(&self,
        captures: &[Capture],
    ) -> std::io::Result<()> {
        fs::create_dir_all(&self.state_dir)?;
        for c in captures {
            let path = self.state_dir.join(format!("{}_{}.hash", c.session_id, c.pane_id));
            fs::write(path, &c.hash)?;
        }
        Ok(())
    }

    fn is_duplicate(&self,
        session_id: &str,
        pane_id: &str,
        hash: &str,
    ) -> bool {
        let path = self.state_dir.join(format!("{}_{}.hash", session_id, pane_id));
    ...
    fn load_prev(&self,
        ...
        let path = self.state_dir.join(format!("{}_{}.prev", session_id, pane_id));
