...
type Config struct {
	...
	CaptureScrollback bool   `yaml:"capture_scrollback"`
	AllowNestedTmux   bool   `yaml:"allow_nested_tmux"`
...
type ConfigWithSource struct {
	...
	CaptureScrollback Source
	AllowNestedTmux   Source
	DiffOnly          Source
	DiffMode          Source
	TextOnly          Source
}
...
func DefaultConfigPath() string {
	...
	return filepath.Join(home, ".config", "terminal-capture-shell.yaml")
}
...
func Load(path string) (*Config, *ConfigWithSource, error) {
	...
	// Overlay YAML file if it exists
	if path != "" {
		...
		// missing file is okay; defaults remain
	}
...
func WriteTemplate(path string) error {
	template := `# terminal-capture-shell configuration
# Environment variables override values in this file.
# Available env vars (names match the config file keys):
#   FLASHBACK_SHELL_SERVER_URL
#   FLASHBACK_SHELL_SOCKET_DIR
#   FLASHBACK_SHELL_SHELL
#   FLASHBACK_SHELL_BUFFER_SIZE
#   FLASHBACK_SHELL_DEVICE_ID
#   FLASHBACK_SHELL_CAPTURE_INTERVAL
#   FLASHBACK_SHELL_DISABLE_CAPTURE
#   FLASHBACK_SHELL_CAPTURE_SCROLLBACK
#   FLASHBACK_SHELL_ALLOW_NESTED_TMUX
#   FLASHBACK_SHELL_DIFF_ONLY
#   FLASHBACK_SHELL_DIFF_MODE
#   FLASHBACK_SHELL_TEXT_ONLY
...
# Allow running flashback-shell inside an existing tmux session by unsetting TMUX
...
# The server receives metadata.ansi=false when this is enabled.
text_only: false
`
...
func LoadConfigAndLogger(configPath, logFile string, verbose int) (*Config, *ConfigWithSource, *log.Logger, error) {
	...
	if err != nil {
		// Fall back to stderr so the user still sees errors.
		out = os.Stderr
		logPath = "stderr"
	}
...
// LoadFromEnv loads the default configuration, applies environment-variable
// overrides, and returns a logger writing to the default log file. No config
// file is read or created.
func LoadFromEnv(verbose int) (*Config, *ConfigWithSource, *log.Logger, error) {
	cfg, src, err := Load("")
	if err != nil {
		return nil, nil, nil, fmt.Errorf("load env config: %w", err)
	}

	out, logPath, err := OpenLogWriter("")
	if err != nil {
		out = os.Stderr
		logPath = "stderr"
	}
	...
	logger := log.New(verbose, out)
	if err != nil {
		logger.Warnf("cannot open default log file %s, logging to stderr: %v", logPath, err)
	}
	logger.Debugf("using log destination: %s", logPath)
...
func openLogWriter(logFile string) (io.Writer, string, error) {
	...
	logDir := filepath.Join(home, ".flashback-shell", "log")
	if err := os.MkdirAll(logDir, 0755); err != nil {
		return nil, logDir, fmt.Errorf("create log dir: %w", err)
	}
	logPath := filepath.Join(logDir, "flashback-shell.log")
...
func defaultConfig() (*Config, *ConfigWithSource) {
	home, _ := os.UserHomeDir()
	cfg := &Config{
		SocketDir:         filepath.Join(home, ".flashback-shell", "tmux"),
		BufferSize:        100,
		CaptureInterval:   30,
		CaptureScrollback: false,
		AllowNestedTmux:   false,
		DiffOnly:          false,
		DiffMode:          "suffix",
		TextOnly:          false,
	}
	src := &ConfigWithSource{
		...
		CaptureScrollback: Source{Value: "false", Origin: "default"},
		AllowNestedTmux:   Source{Value: "false", Origin: "default"},
...
func applyYAML(cfg *Config, src *ConfigWithSource, fileCfg *FileConfig) {
	...
	if fileCfg.CaptureScrollback != nil {
		cfg.CaptureScrollback = *fileCfg.CaptureScrollback
		src.CaptureScrollback = Source{Value: fmt.Sprintf("%t", cfg.CaptureScrollback), Origin: "config"}
	}
	if fileCfg.AllowNestedTmux != nil {
		cfg.AllowNestedTmux = *fileCfg.AllowNestedTmux
		src.AllowNestedTmux = Source{Value: fmt.Sprintf("%t", cfg.AllowNestedTmux), Origin: "config"}
	}
...
func applyEnvOverrides(cfg *Config, src *ConfigWithSource) {
	if v := os.Getenv("FLASHBACK_SHELL_SERVER_URL"); v != "" {
		cfg.ServerURL = v
		src.ServerURL = Source{Value: v, Origin: "env"}
	}
	if v := os.Getenv("FLASHBACK_SHELL_SOCKET_DIR"); v != "" {
		cfg.SocketDir = v
		src.SocketDir = Source{Value: v, Origin: "env"}
	}
	if v := os.Getenv("FLASHBACK_SHELL_SHELL"); v != "" {
		cfg.Shell = v
		src.Shell = Source{Value: v, Origin: "env"}
	}
	if v := os.Getenv("FLASHBACK_SHELL_BUFFER_SIZE"); v != "" {
		if n := parseInt(v); n > 0 {
			cfg.BufferSize = n
			src.BufferSize = Source{Value: v, Origin: "env"}
		}
	}
	if v := os.Getenv("FLASHBACK_SHELL_DEVICE_ID"); v != "" {
		cfg.DeviceID = v
		src.DeviceID = Source{Value: v, Origin: "env"}
	}
	if v := os.Getenv("FLASHBACK_SHELL_CAPTURE_INTERVAL"); v != "" {
		if n := parseInt(v); n >= 0 {
			cfg.CaptureInterval = n
			src.CaptureInterval = Source{Value: v, Origin: "env"}
		}
	}
	if v := os.Getenv("FLASHBACK_SHELL_DISABLE_CAPTURE"); v != "" {
		cfg.DisableCapture = parseBool(v)
		src.DisableCapture = Source{Value: v, Origin: "env"}
	}
	if v := os.Getenv("FLASHBACK_SHELL_CAPTURE_SCROLLBACK"); v != "" {
		cfg.CaptureScrollback = parseBool(v)
		src.CaptureScrollback = Source{Value: v, Origin: "env"}
	}
	if v := os.Getenv("FLASHBACK_SHELL_ALLOW_NESTED_TMUX"); v != "" {
		cfg.AllowNestedTmux = parseBool(v)
		src.AllowNestedTmux = Source{Value: v, Origin: "env"}
	}
	if v := os.Getenv("FLASHBACK_SHELL_DIFF_ONLY"); v != "" {
		cfg.DiffOnly = parseBool(v)
		src.DiffOnly = Source{Value: v, Origin: "env"}
	}
	if v := os.Getenv("FLASHBACK_SHELL_DIFF_MODE"); v != "" {
		cfg.DiffMode = v
		src.DiffMode = Source{Value: v, Origin: "env"}
	}
	if v := os.Getenv("FLASHBACK_SHELL_TEXT_ONLY"); v != "" {
