...
func parseArgs(osArgs []string) (verbose int, configPath, logFile string, noCapture bool, command string, subArgs []string, err error) {
	...
	// Examples:
	//   flashback-shell -vvv new
	//   flashback-shell -c /tmp/cfg.yaml -l /tmp/log.txt capture
	//   flashback-shell --no-capture new -c 'echo hello'
...
func printUsage() {
	fmt.Println("flashback-shell - Terminal session capture tool")
	fmt.Println()
	fmt.Println("Usage:")
	fmt.Println("  flashback-shell [global flags] <command> [command args]")
	fmt.Println()
	fmt.Println("Commands:")
	fmt.Println("  new [args...]              Start a new shell (tmux if available, else fallback)")
	fmt.Println("  capture                    Capture all tmux sessions and upload changes")
	fmt.Println("  list                       List managed tmux sessions")
	...
	fmt.Println("  -l /dev/stdout           Log to stdout (may corrupt tmux attach)")
	...
	fmt.Println("  FLASHBACK_SHELL_ALLOW_NESTED_TMUX   allow_nested_tmux (true/false)")
	...
	fmt.Println("  disable_capture, capture_scrollback, allow_nested_tmux, diff_only,")
	fmt.Println("  diff_mode, text_only")
	fmt.Println()
